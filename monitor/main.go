...
func (s Slack) BuildURL(token string) string {
	return fmt.Sprintf("%s?token=%s", SLACK_URL, token)
}
...
type SlackPrescenseChange struct {
	Presence string `json:"presence"`
	User     string `json:"user"`
}
...
func SlackConnect(token string, channelTarget string) {
	...
	slack.WebsocketConn, _, err = Dialer.Dial(slack.Details.URL, http.Header{})

	if err != nil {
		log.Printf("%s Unable to connect to Websocket. Error: %s", err)
	}
